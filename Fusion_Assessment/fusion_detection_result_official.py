...
[1]C. Li, T. Yang, S. Zhu, C. Chen, and S. Guan, “Density map guided
object detection in aerial images,” in 2020 IEEE/CVF Conference on
...
if __name__ == "__main__":
    ...
    img_gt_file = os.path.join(root, folder_name, mode, "Global", "test.json")
    img_detection_file = os.path.join(root, folder_name, mode, "Global_swin_tiny_test_results.bbox.json")
    dens_gt_file = os.path.join(root, folder_name, mode, "Density", mode + ".json")
    dens_detection_file = os.path.join(root, folder_name, mode, "Density_swin_tiny_test_results.bbox.json")
    ...
    for img_id in tqdm(cocoGt_global.getImgIds(), total=len(img_list)):
        ...
        for dens_img_id, dens_fullname in enumerate(matched_dens_file):
            ...
            # 确保 dens_name 是文件名，而不是路径的一部分
            dens_name = os.path.basename(dens_fullname)
    ...
    for i in tqdm(cocoGt_global.getImgIds(), total=len(img_list)):
        ...
        img_name = global_img[0]["file_name"]
        if not current_nms_target:  # 如果 current_nms_target 为空，跳过
            continue
